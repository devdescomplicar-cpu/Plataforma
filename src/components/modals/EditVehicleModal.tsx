...
import { Car, Upload, X, Loader2, Calendar as CalendarIcon, Plus, Star, GripVertical } from 'lucide-react';
...
import { useUpdateVehicle, useVehicle, VehicleResponse } from '@/hooks/useVehicles';
...
export function EditVehicleModal({ open, onOpenChange, vehicleId }: EditVehicleModalProps) {
  ...
  const vehicleTypeForFipe = formData.vehicleType === 'car' ? 'carros' : 'motos';
  ...
  interface ImageItem {
    id?: string; // ID da imagem existente no banco
    file?: File; // Arquivo novo (quando adicionado)
    preview: string; // URL de preview
    isCover: boolean; // Se é a imagem de capa
    key?: string; // Key do MinIO (para imagens existentes)
  }
  ...
  // Carregar dados do veículo quando o modal abrir
  useEffect(() => {
    if (vehicleData && open && vehicleId && vehicleData.id === vehicleId && !dataLoaded) {
      const v = vehicleData as any;
      
      setFormData({
        vehicleType: (v.vehicleType || 'car') as 'car' | 'motorcycle',
        brand: vehicleData.brand || '',
        brandId: '', // Não temos ID salvo, usuário precisará selecionar novamente
        model: vehicleData.model || '',
        modelId: '', // Não temos ID salvo, usuário precisará selecionar novamente
        isCustomModel: true, // Assumir como customizado já que não temos ID
        year: vehicleData.year?.toString() || '',
        yearId: '', // Não temos ID salvo, usuário precisará selecionar novamente
        isCustomYear: true, // Assumir como customizado já que não temos ID
        km: v.km?.toString() || '',
        ...
        steering: v.steering || '',
        origin: (v.origin || 'own') as 'own' | 'consignment' | 'repass',
        features: Array.isArray(v.features) ? v.features : [],
        purchasePrice: vehicleData.purchasePrice
          ? formatCurrencyValue(vehicleData.purchasePrice)
          : '',
        salePrice: vehicleData.salePrice ? formatCurrencyValue(vehicleData.salePrice) : '',
        fipePrice: v.fipePrice ? formatCurrencyValue(v.fipePrice) : '',
        description: vehicleData.description || '',
        consignmentOwnerName: v.consignmentOwnerName || '',
        consignmentOwnerPhone: v.consignmentOwnerPhone || '',
        consignmentCommissionType: (v.consignmentCommissionType || '') as 'percentual' | 'fixed' | '',
        consignmentCommissionValue: v.consignmentCommissionValue
          ? formatCurrencyValue(v.consignmentCommissionValue)
          : '',
        consignmentMinRepassValue: v.consignmentMinRepassValue
          ? formatCurrencyValue(v.consignmentMinRepassValue)
          : '',
      ...
      if (vehicleData.images && vehicleData.images.length > 0) {
        const imageItems: ImageItem[] = vehicleData.images.map((img, index) => ({
          id: img.id,
          preview: toPublicImageUrl(img.url) || img.url || '',
  ...
  }, [vehicleData, open, vehicleId, dataLoaded]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.brand || !formData.model || !year || !formData.fuel || !formData.color) {
      toast({
        ...
        description: 'Preencha marca, modelo, ano, combustível e cor.',
        variant: 'destructive',
      });
      return;
    }
    ...
    if (formData.consignmentCommissionValue) {
      if (formData.consignmentCommissionType === 'fixed') {
        commissionValue = parseCurrency(formData.consignmentCommissionValue);
      } else {
        commissionValue = parseFloat(formData.consignmentCommissionValue);
      }
    }
    
    const minRepassValue = formData.consignmentMinRepassValue
      ? parseCurrency(formData.consignmentMinRepassValue)
      : undefined;
    ...
    // Identificar imagens que foram removidas (comparar com imagens originais)
    if (vehicleData?.images) {
      const currentImageIds = existingImages.map(img => img.id).filter((id): id is string => !!id);
      const originalImageIds = vehicleData.images.map(img => img.id);
      imagesToDelete.push(...originalImageIds.filter(id => !currentImageIds.includes(id)));
    }

    // Preparar novas imagens para upload
    const imageFilesToSend = newImages.map((img) => img.file!);

    // Preparar ordem de TODAS as imagens (existentes + novas) na ordem atual do array
    // A ordem é baseada na posição atual no array `images`
    const imageOrder = images
      .map((img, idx) => {
        // Apenas imagens existentes têm ID e precisam ser reordenadas
        if (img.id) {
          return {
            id: img.id,
            order: idx,
    ...
    // Sempre enviar imageOrder se houver imagens existentes, mesmo que não haja novas imagens ou remoções
    // Isso garante que a reordenação seja aplicada
    const hasExistingImages = images.some(img => img.id && !img.file);
    const shouldSendImageOrder = hasExistingImages && imageOrder.length > 0;
  ...
  if (isLoadingVehicle || (open && vehicleId && !vehicleData)) {
    return (
  ...
  // Reutilizar o mesmo formulário do AddVehicleModal, mas com dados pré-preenchidos
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <DialogTitle className="text-xl">
                Editar {formData.vehicleType === 'motorcycle' ? 'Moto' : 'Carro'}
              </DialogTitle>
              <DialogDescription>
              ...
              {formData.isCustomModel ? (
                <div className="flex gap-2">
                  <Input
                    ...
                    placeholder={formData.vehicleType === 'motorcycle' ? "Ex: CB 600F, XRE 300..." : "Ex: Civic, Corolla..."}
                  />
                  <Button
                    type="button"
                    variant="outline"
